...
if "Доступно обновление TermuGram!" in t:
    ...
    send("\n")  # Enter -> в меню
    clear_buf()  # убрать текст уведомления, иначе он «перекрывает» шаги ниже
...
# 9. Меню -> Проверить обновления (индекс 6)
check(read_until(["главное меню"]), "вернулись в меню")
down()
down()
down()
...
down()
send("\n")
if read_until(["У вас уже последняя версия", "Доступная версия", "Не удалось проверить"], timeout=45):
    settle()
    t = clean_text()
    if "Доступная версия" in t:
        check(True, "ручная проверка нашла новую версию")
        down()  # выбрать «Отмена»
        send("\n")
        check(read_until(["главное меню"]), "вернулись в меню после ручной проверки")
    else:
        check(True, "ручная проверка: последняя версия" if "У вас уже последняя версия" in t else "нет связи")
        send("\n")
        check(read_until(["главное меню"]), "вернулись в меню")
else:
    check(False, "экран проверки обновлений не появился")
# 10. Выход (индекс 7)
check(read_until(["главное меню"]), "вернулись в меню")
down()
down()
